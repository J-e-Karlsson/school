...
def get_batch(source, i):
    seq_len = min(args.length_seq, len(source) - 1 - i) #set length of the sequence, smallest between the two parameters to stay within data
    data = source[i:i+seq_len] #slices of seq_len size starting from i
    target = source[i+1:i+seq_len+1].view(-1) #same as above but i+1, so shifted one step ahead to link data with next words
    return data, target
...
def train():
    ...
    for batch, i in enumerate(range(0, train_data.size(0) - 1, args.length_seq)):
        ...
        data, targets = get_batch(train_data, i) #calls batch function, assigns data and targets variables
        model.zero_grad()  # What does this do? - this resets the gradients to zero each step, avoids influences from previous iterations
        hidden = repackage_hidden(hidden) #cut history to only update hidden states from the last batch, not to backpropagate through whole history, saves memory
        
        # TODO: given the data, compute the gradients to update the model
        #  1) forward computation
        normalized, hn = model(data, hidden) #call the model class, pytorch calls forward

        # 2) compute the loss
        loss = criterion(normalized, targets) #take the softmaxed data from the forward computation and batch targets to compute loss
        
        # 3) backpropagation -> gradients
        loss.backward() #pytorch backward method on loss tensor, creates gradient in parameter.grad

        # clip_grad_norm helps to prevent the gradient explosion in RNNs
        torch.nn.utils.clip_grad_norm_(model.parameters(), args.clip)
        
        with torch.no_grad():
            for p in model.parameters(): #this is a manual build of .optim (for example) and .step
                # TODO: update model parameters based on gradients
                p -= p.grad*lr #SGD to minimize loss
