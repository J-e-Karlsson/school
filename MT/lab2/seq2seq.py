...
class EncoderRNN(nn.Module):
    ...
    def __init__(self, input_size, hidden_size):
        ...
        # TODO 1: Initialize your word embedding, encoder rnn
        self.emb = nn.Embedding(input_size, self.hidden_size) #stores embeddings, takes index list     
        self.rnn = nn.GRU(self.hidden_size, self.hidden_size, num_layers=1) #instantiate RNN model, hidden_size twice because that is the size of the input and output
        #self.rnn always sees receives input of hidden_size, since the embedded input has the same size as hidden
    ...
    def forward(self, input, hidden):
        ...
        embed_input = self.emb(input.view(1, 1)) #embed the input using embedder. "view" adds the batch dimension [1] to line up with hidden. 1,256 ---> 1,1,256
        print("AFTER:",embed_input.size())
        output, hidden = self.rnn(embed_input, hidden) #creates output and hidden layer from embeddings and hidden
...
class DecoderRNN(nn.Module):
    ...
    def __init__(self, hidden_size, output_size):
        ...
        # TODO 3: Initialize your word embedding, decoder rnn, output layer, softmax layer
        #  similar to the NLM model in Assignment3
        self.emb = nn.Embedding(output_size, self.hidden_size) #stores embeddings, takes index list  
        self.rnn = nn.GRU(self.hidden_size, self.hidden_size, num_layers=1) #instantiate same as encoder but use output_size instead
        self.generator = nn.Linear(self.hidden_size, output_size) #Linear layer for logits
        self.softmax = nn.LogSoftmax(dim=2) #softmax function with dim 2 as default to catch the hidden size
    ...
    def forward(self, input, hidden):
        ...
        embed_input = self.emb(input.view(1, 1)) #embed the input using embedder. "view" adds the batch dimension [1] to line up with hidden. 1,256 ---> 1,1,256
        #print("AFTER: ", embed_input.size())
        output, hidden = self.rnn(embed_input, hidden) #embedding and hidden layer match in size
        logits = self.generator(output) #linear layer for logits to give to softmax
        log_softmax = self.softmax(logits) #applies softmax on dim = 2 which is the class dimension of hidden size 1361 ([1,1,1361])
...
def train(input_tensor, target_tensor, encoder, decoder, optimizer, criterion):
    ...
    for ei in range(input_length):
        # TODO 5: feed each input to the encoder, and get the output | Already done
    ...
    # TODO 5: initialize the decoder with the last encoder hidden state | Already done
    ...
    for di in range(target_length):
        ...
        loss += criterion(decoder_output, target_tensor[di]) #compute loss using the negative log likelihood loss. [di] indexes the correct choice in target tensor
...
def main():
    ...
    ap.add_argument('--load_checkpoint', nargs=1, default=None,
                    help='checkpoint file to start from')
    ap.add_argument('--inference', default=True, action='store_true')
